...
function writePathAlgebra(algebra) {
  ...
  if (algebra.type === 'bgp' && algebra.patterns.every(quad => quad.predicate.termType === 'NamedNode') && algebra.patterns.length >= 0) return algebra.patterns.map(quad => `<${quad.predicate.value}>`).join('/');
...
class ComplexPathResolver extends _AbstractPathResolver.default {
  ...
  async lookupProperty(property) {
    ...
    if (algebra.input.type === 'bgp' && algebra.input.patterns.length === 1 && algebra.input.patterns[0].predicate.termType === 'NamedNode') return (0, _dataModel.namedNode)(algebra.input.patterns[0].predicate.value);
