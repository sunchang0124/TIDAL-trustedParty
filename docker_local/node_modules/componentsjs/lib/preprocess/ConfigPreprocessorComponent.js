 ...
 */
class ConfigPreprocessorComponent {
    constructor(options) {
    ...
    canHandle(config) {
        if (!config.property.requireName) {
            ...
            if (!module) {
                throw new ErrorResourcesContext_1.ErrorResourcesContext(`No module was found for the component "${component.value}"`, { config });
            }
    ...
    transform(config, handleResponse) {
        ...
        if (!requireName) {
            throw new ErrorResourcesContext_1.ErrorResourcesContext(`Could not find a requireName in either the config's module or component`, {
     ...
     */
    inheritParameterValues(config, component) {
        ...
        for (const parameter of component.properties.parameters) {
            ...
            const inheritanceValueDefinitions = parameter.properties.inheritValues
                .reduce((acc, clazz) => {
                if (clazz.properties.types.reduce((subAcc, type) => subAcc ||
                    type.value === Iris_1.IRIS_OWL.Restriction, false)) {
                    acc.push(clazz);
                }
                return acc;
            }, []);
            ...
            for (const inheritanceValueDefinition of inheritanceValueDefinitions) {
                ...
                if (inheritanceValueDefinition.property.from) {
                    ...
                    if (!inheritanceValueDefinition.property.onParameter) {
                        throw new ErrorResourcesContext_1.ErrorResourcesContext(`Missing onParameter property on parameter value inheritance definition`, {
                            parameter,
                            config,
                            component,
                        });
                    ...
                    for (const componentType of inheritanceValueDefinition.properties.from) {
                        if (componentType.type !== 'NamedNode') {
                            throw new ErrorResourcesContext_1.ErrorResourcesContext(`Detected invalid from term type "${componentType.type}" on parameter value inheritance definition`, {
                                parameter,
                                config,
                                component,
                            });
                        ...
                        const typeInstances = this.runTypeConfigs[componentType.value];
                        if (typeInstances) {
                            for (const instance of typeInstances) {
                                ...
                                for (const parentParameter of inheritanceValueDefinition.properties.onParameter) {
                                    if (parentParameter.type !== 'NamedNode') {
                                        throw new ErrorResourcesContext_1.ErrorResourcesContext(`Detected invalid onParameter term type "${parentParameter.type}" on parameter value inheritance definition`, {
                                            parentParameter,
                                            config,
                                            component,
                                        });
                    ...
                    throw new ErrorResourcesContext_1.ErrorResourcesContext(`Missing from property on parameter value inheritance definition`, {
                        parameter,
                        config,
                        component,
                    });
     ...
     */
    validateConfig(config, handleResponse) {
